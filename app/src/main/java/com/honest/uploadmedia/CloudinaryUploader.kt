...
class CloudinaryUploader(private val context: Context) {
    ...
    init {
        ...
        config["cloud_name"] = "dy9scmo1m"
        config["api_key"] = "454234546361358"
        config["api_secret"] = "wQTZeUdsTS89sqSHPqqo9PkWqqY"
        MediaManager.init(context, config)
    }

    fun uploadMedia(uri: Uri, isVideo: Boolean, callback: UploadCallback) {
    ...
    private fun createTempFileFromUri(uri: Uri, isVideo: Boolean): File? {
        try {
